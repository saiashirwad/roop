...
const main = Effect.gen(function* () {
  const apiKey = yield* Config.string("DEEPSEEK_API_KEY").pipe(Config.option)
  ...
  return yield* Layer.launch(server({ port, root, deepseekApiKey: Option.getOrUndefined(apiKey) }))
})
